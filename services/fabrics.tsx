...
export const getList = async (token:string | undefined): Promise<{id:string;name:string;color:string;type:string,unit:string,price:number}[]> => {
    const { data } = await axiosInstanceServer.get(ApiRouter.FABRICS,{
            headers: {
                'Authorization': `Bearer ${token}`
            }
        });
    return data as {id:string;name:string;color:string;type:string,unit:string,price:number}[];
}

export const create = async (req:{name:string;color:string;type:string,unit:string,price:number}): Promise<{id:string;name:string;color:string;type:string,unit:string,price:number}> => {
    const { data } = await axiosInstance.post(ApiRouter.FABRICS,req);
    return data as {id:string;name:string;color:string;type:string,unit:string,price:number};
}


export const update = async (id:string,req:{name?:string;color?:string;type?:string,unit?:string,qty?:number,price?:number}): Promise<{id:string;name:string;color:string;type:string,unit:string,price:number}> => {
