...
export default function ThreadsCreateModal({children,onSubmit}:{children:React.ReactNode,onSubmit:(id:string,color:string,type:string,unit:string,qty:number,price:number,)=>void}) {
  ...
  const handleSubmit = async () => {
    ...
    try {
        const response = await Api.threads.create(thread)
        if(response.id){
            setColor("");
            setType("");
            setUnit("");
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>Создание материала - Нитки</DialogTitle>
        </DialogHeader>
