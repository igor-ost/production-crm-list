...
export default function ZippersCreateModal({children,onSubmit}:{children:React.ReactNode,onSubmit:(id:string,color:string,type:string,unit:string,qty:number,price:number,)=>void}) {
  ...
  const handleSubmit = async () => {
    ...
    try {
        const response = await Api.zippers.create(zipper)
        if(response.id){
            setColor("");
            setType("");
            setUnit("");
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>Создание материала - Молния</DialogTitle>
        </DialogHeader>
