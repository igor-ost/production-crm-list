...
export default function AccessoriesCreateModal({children,onSubmit}:{children:React.ReactNode,onSubmit:(id:string,name:string,unit:string)=>void}) {
  ...
  const handleSubmit = async () => {
    ...
    try {
        const response = await Api.accessories.create(accessories)
        if(response.id){
            setName("");
            setUnit("");
            onSubmit(response.id,name,unit)
            setIsLoading(false)
            setOpen(false)
        }
    } catch (error:unknown) {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>Создание материала - Аксессуары</DialogTitle>
        </DialogHeader>
