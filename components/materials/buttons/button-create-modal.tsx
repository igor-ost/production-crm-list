...
export default function ButtonsCreateModal({children,onSubmit}:{children:React.ReactNode,onSubmit:(id:string,color:string,type:string,unit:string,qty:number,price:number,)=>void}) {
  ...
  const handleSubmit = async () => {
    ...
    try {
        const response = await Api.buttons.create(button)
        if(response.id){
            setColor("");
            setType("");
            setUnit("");
            setQty(0);
            setPrice(0);
            onSubmit(response.id,color,type,unit,qty,price)
            setIsLoading(false)
            setOpen(false)
        }
    } catch (error:unknown) {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>Создание материала - Пуговицы</DialogTitle>
        </DialogHeader>
