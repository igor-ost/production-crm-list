...
export default function TemplatesRemoveItemsModal({
  onSubmit,
  id,
  ...
  const handleConfirm = async () => {
    setIsLoading(true)
    try {
        const response = await Api.template_items.remove(id)
        if(response.status){
            setIsLoading(false)
            setOpen(false)
            onSubmit(id)
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
          <div>
            <Trash2 className="w-3 h-3 z-10 opacity-0 group-hover:opacity-100 text-red-600" />
          </div>
      </DialogTrigger>
